...
const messaging_message_3 = require("../../../models/messaging-message");
...
module.exports = (params, user) => new Promise(async (res, rej) => {
    ...
    // Serialize
    const messageObj = await messaging_message_3.pack(message);
    // Reponse
    res(messageObj);
    // 自分のストリーム
    event_2.publishMessagingStream(message.user_id, message.recipient_id, 'message', messageObj);
    event_2.publishMessagingIndexStream(message.user_id, 'message', messageObj);
    ...
    event_2.publishMessagingIndexStream(message.recipient_id, 'message', messageObj);
    ...
    setTimeout(async () => {
        ...
        if (!freshMessage.is_read) {
            ...
            event_2.pushSw(message.recipient_id, 'unread_messaging_message', messageObj);
        }
    }, 3000);
