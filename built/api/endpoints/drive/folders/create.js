...
module.exports = (params, user) => new Promise(async (res, rej) => {
    // Get 'name' parameter
    const [name = '無題のフォルダー', nameErr] = cafy_1.default(params.name).optional.string().pipe(drive_folder_1.isValidFolderName).$;
    if (nameErr)
    ...
    // Serialize
    const folderObj = await drive_folder_1.pack(folder);
    // Response
    res(folderObj);
    // Publish folder_created event
    event_1.publishDriveStream(user._id, 'folder_created', folderObj);
});
