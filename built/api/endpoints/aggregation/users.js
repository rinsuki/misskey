...
module.exports = params => new Promise(async (res, rej) => {
    ...
    for (let i = 0; i < limit; i++) {
        ...
        // day = day.getTime();
        const total = users.filter(u => u.created_at < dayEnd && (u.deleted_at == null || u.deleted_at > dayEnd)).length;
        const created = users.filter(u => u.created_at < dayEnd && u.created_at > dayStart).length;
        graph.push({
            total: total,
            created: created
        });
