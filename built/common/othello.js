...
class Othello {
    ...
    getPattern(myColor) {
        ...
        this.board.forEach((stones, y) => stones.forEach((stone, x) => {
            if (stone != null)
                return;
            if (this.canReverse(myColor, x, y))
                result.push([x, y]);
        }));
        return result;
    }
     ...
     */
    canReverse(myColor, targetx, targety) {
        return this.getReverse(myColor, targetx, targety) !== null;
    }
    getReverse(myColor, targetx, targety) {
...
exports.default = Othello;
function ai(color, othello) {
